/*
    ...
    Description: 
        1.判断是否存在购物车缓存信息
        2.若存在，更新菜品分类、菜品、购物车
        3.若不存在，获取菜品分类，执行事件绑定
        4.滑动时，滑动到某一菜品分类临界值时，需要去请求下一菜品数据  todo
 */
...
Mo.ready(function(M) {
    ...
    var DOM_KOUBEI_Orders=M.role("koubei-orders"), // 口碑 订单
        DOM_KOUBEI_User = M.role("koubei-user"); // 口碑 我的
    ...
    try {
        var url = {
            GetCategoryList: "/dine/categorylist", // 获取菜品分类
            GetDishList: "/dine/dishlist", // 根据分类Id获取菜品
            GetShoppingCart: "/dine/mycart", // 获取购物车
            AddDish: "/dine/adddish", // 添加菜品
            ClearCartDish: "/dine/clearcart", // 清除购物车
            ...
            isKoubei:M.one("input[name='hidIsKoubei']").getAttr("value")=="true"?true:false // 是否是口碑
            // IsSelfPick: M.one("input[name='hidIsCanPickUp']").getAttr("value"), // 判断是否开通自提
    ...
    // 8.判断是否超区
    function isInDeliveryScope(con) {
        M.xPost({
    ...
    // 9.是否打烊
    function isOpenDoor(con) {
        con.whetherOpen = url.IsOpenDoor;
        handleTips(con);
    }

    // 10.处理提示信息
    function handleTips(con) {
        ...
        if (!con.whetherOpen) {
            ...
            M.dialog(message, function() {}, 'tips', 'hide');
            if(M.one(".title")){
    ...
    //购物车信息更新，flag标志位，flag为1，标志为首次加载
    function updatePage(data, flag) {
        ...
        if (data.totalCount != 0) {
            ...
            for (var i = 0; i < data.cart.categoryList.length; i++) {
                ...
                // var currentCategoryId = currentCategory.getAttr("data-cid");
                // if (currentCategoryId == data.cart.categoryList[i].categoryId) {
                //     setMenuNum(currentCategoryId, data.cart.categoryList[i].count);
                //     var dishList = data.cart.categoryList[i].dishList;
                //     for (var j = 0; j < dishList.length; j++) {
                //         setFoodNum(dishList[j].dishId, dishList[j].count);
                //     }
                // }
            }
        }
    ...
    function updateDish() {
        ...
        var headerHeight=0;
        if(M.one('.header')){
            headerHeight=M.one('.header').get('region').height
        }
        var seeHeigh = M.one(window).get('region').height - M.one('.ft-bar').get('region').height - headerHeight;
        if (M.one('.adr-bar')) {
    ...
    M.one(".sub-opc").on("click", function() {
         if(url.isKoubei){
            window.location.href = "/dine/koubei/shoppingcar";
        }else{
            window.location.href = "/dine/shoppingcar";
        }
    });
    ...
    // 口碑跳转的页面
    if(DOM_KOUBEI_Orders){
        DOM_KOUBEI_Orders.on("click",function(){
            window.location.href="/koubei/myorder/dineOrderList";
        });
    }

    if(DOM_KOUBEI_User){
        DOM_KOUBEI_User.on("click", function(){
            window.location.href="/koubei/my";
        });
    }
