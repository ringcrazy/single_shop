 ...
 */
//@require jscroll,amount-plugin,mask,confirm,swiper,template,inscreen,location,localstorage
Mo.ready(function(M) {
    ...
    try {
        var url = {
            GetCategoryList:"http://10.8.7.200:3000/waimai/categorylist"
    ...
    var DOM_MENU_WRAP = M.one('.nav-lst'); // 左侧 菜品分类容器
    var pos = [];
    ...
    var currentFood = DOM_FOOD.one(".active");
    ...
    DOM_MENU_WRAP.all('li').on('click', function() {
        ...
        // 更新菜品
        currentFood && currentFood.removeClass("active").addClass("hide");
        currentFood = DOM_FOOD.one('[data-cid=' + cid + ']');
        if (currentFood) {
            currentFood.addClass("active").removeClass("hide");
